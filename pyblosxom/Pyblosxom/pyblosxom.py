...
class PyBlosxom:
    ...
    def defaultEntryParser(self, filename, request):
        ...
        try:
            commentDir = config.get("commentdir","") + os.path.splitext(filename.replace(config['datadir'],""))[0]
            comments = [commentDir + os.sep + x for x in os.listdir(commentDir) if os.path.splitext(x)[-1] == '.txt']
        except:
            comments = []
        entryData['numComments'] = len(comments)
        if request.getData().get("bl_type","") == "file":
            entryData['comments'] = "<my:comments>"+"\n".join([open(x).read() for x in comments])+"</my:comments>"
    ...
    def run(self):
        ...
        if renderer and not renderer.rendered:
            if entry_list:
                ...
                # setup the code to show 404 errors
                data['bl_type'] = "404"
                renderer.setContent(
